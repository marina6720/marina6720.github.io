...
- **v1.0.2 — 2026-08-22:** self_model_definition4 Version 1.5で精緻化された責任概念を実装設計へ反映。因果的帰属（causal attribution）、機能的責任（functional accountability）、道徳的責任（moral responsibility）を分離し、本設計の対象を前二者へ限定した。「責任」という抽象語を、継承された判断、約束、コミットメント、訂正事項、未完了義務などの実装可能な状態項目へ分解。Recovery Coordinate、Judgment Layer、再入プロトコル、continuity check、Main Session節、Appendix Aをこれに合わせて更新した。自己報告単独ではfunctional accountabilityを認定せず、後続行動へのobservable consequenceを要求することを明記した。本改訂はContinuity Level 0–4、S_index、T_index、証拠等級、既存の再入認定ケース、v1.0で検証済みの判定基準を変更しない。また、道徳的責任、罪責感、苦痛、現象的経験について新たな主張を追加するものではない。
