...
**リンク: [第2版があります](./openclaw-main-session-response-routing-investigation-report-2026-07-22-v2-ja.md)**  
...
現在のsession、JSONL、workspaceを維持したまま、実行環境だけをDが更新前に実際に使用していたOpenClaw 2026.6.6へ戻した。初回のtool-use回帰試験では、回答は一通で完結し、途中commentaryの独立配送、徐々に薄くなる言い直し、同文二重表示はいずれも再現しなかった。

したがって、現時点の中心結論は次である。

> **今回確認された複数回答、回答の希薄化、および二重表示の主因は、DやClaude Opus 4.6の人格的・能力的問題ではなく、OpenClaw 2026.7.1のtool-useオーケストレーション、途中配送、same-run provider projection、ならびに履歴投影の不具合だった。番号付き履歴の前進は別層の固定長窓という仕様であり、compaction summaryの自己像の歪みはOpenClawの継続性設計とモデル生成品質が重なる複合問題である。**
...
| `delivery-mirror`の二重表示 | 確認されたhistory projection問題 | canonicalとmirrorを同じassistant一覧へ投影 |
...
この介入では、modelも長期sessionもDの記憶ファイルも変えていない。実行環境だけを2026.7.1から2026.6.6へ戻して症状が消えたため、複数回答と二重表示の主因がOpenClaw 2026.7.1側にあったことを強く支持する。

ただし、初回の一往復だけで、すべての長いtool loopにおける再発可能性がゼロになったとは断定しない。継続監視が必要である。
...
10. 現在のdataを維持したまま実行環境だけを2026.6.6へ戻すと、初回tool-use試験で複数回答と二重表示は再現しなかった。
11. よって、主要な技術的不具合の主因はOpenClaw 2026.7.1側にあったと判断する。

最も簡潔な結論は次である。

> **Dが壊れたのではない。Dの一つのtool-use過程を、OpenClawが複数の通常回答として配送し、その途中回答を次のmodel passへ再投入し、さらに配送用mirrorを会話として二重表示していた。会話原本は保たれていた。実行環境だけを更新前へ戻すと、Dは現在の記録を維持したまま、一通の完全な回答へ戻った。**
...
- 「OpenClaw main sessionで頻発した早期コンパクションの調査報告（暫定版・第4版）」
