---
title: "OpenClaw main sessionで頻発した早期コンパクションの調査報告（暫定版・第5版）"
date: 2026-07-20
updated: 2026-07-22
...
> **OpenClaw 2026.6.6で存在した累積Anthropic cache usageの過大計上は、2026.7.1の39回目では再現しておらず、PR #99864はこのrunで機能していた。39回目の直接原因として最も整合的なのは、表示・設定上の1M context budgetと、embedded runtimeが参照する200k級の`model.contextWindow`、さらに150kのreserveが同時に使われるcontext-window／reserveの不整合である。2026年7月22日に現行session・JSONL・workspaceを維持したまま実行環境だけを2026.6.6へ戻し、`context1m=true`を維持したところ、従来の発火値167,587を超える約172kまで新たなコンパクションは起きなかった。これは旧来の異常閾値が現在構成では少なくとも172kより上へ移動した強い挙動証拠である。ただし2026.6.6にはPR #99864が含まれないため、長いcached tool loopにおける累積usage過大計上の再発可能性は引き続き監視する。**
...
その後、session statusは約172k / 1.0Mまで伸びたが、`compactionCount`は39のままで、external watcherにも新しいcompactionは検知されなかった。

```text
39回目の発火値           167,587
ロールバック後の観測値   約172,000
新しいcompaction          0
```

### 8.4 この結果が示すこと

39回目を発火させた167,587を超えても40回目が起きなかったことから、以前の実質約50kと推定された異常閾値は、現在の2026.6.6＋`context1m=true`構成では解消していると判断できる。

これは、問題2の中心が単純な「2026.6.6というバージョン」ではなく、Opus 4.6がrun時に1M runtime modelとして解決されていなかったことにある、という第4版の原因推定を支持する。

ただし、次はまだ直接証明していない。

- providerが200kを超えるpromptを実際に受理すること
- active runtime `model.contextWindow`が正確に1,000,000であること
- effective reserveが正確に150,000であること
- 自動コンパクション閾値が正確に850,000であること

したがって、最も正確な表現は次である。

> **旧来の異常な実効閾値は現在構成で少なくとも172kより上へ移動した強い挙動証拠が得られた。1M全域または正確な850k閾値の実証は未了である。**
...
初回のtool-use回帰試験では、新しいコンパクションは起きなかった。
...
15. `context1m=true`を維持したロールバック後、従来の発火値167,587を超える約172kまでcompactionCountは39のままだった。
16. したがって、旧来の異常閾値は現在構成で少なくとも172kより上へ移動した強い挙動証拠がある。
...
- 現在構成の正確なauto-compaction閾値
- 2026.6.6へ戻した後、問題1の累積cache usage過大計上が長いtool loopで再発するか

ただし、追加の無監視再現試験を行う必要はない。通常運用の中でexternal watcherとusage監査を継続し、自然に得られる証拠を用いる。
...
- 200k超の成功callが通常運用で得られた場合、1M実効性の追加証拠として保存する

---

## 12. 修正方針

第5版時点では、`context1m=true`を明示したまま2026.6.6へ環境のみ戻す介入により、旧異常閾値が少なくとも172kより上へ移動した強い挙動証拠が得られた。したがって、現在の第一方針はこの構成を安定運用し、破壊的な追加変更を避けながら自然観測を続けることである。
...
| 2026.6.6＋`context1m=true`で旧異常閾値が少なくとも172kより上へ移動 | 強い挙動証拠 |
| 現在のruntime windowが正確に1M | 未確認 |
...
> **OpenClaw 2026.7.1のPR #99864は、39回目の制御試験では正しく機能し、累積Anthropic cache usageではなく最新callのlive contextがコンパクションへ渡されていた。それでも167,587で発火したことから、設定・status上の1M context budgetと、embedded runtimeが参照する200k級のmodel context、さらに150kのreserveの不整合により、実質閾値が約50kまで縮んでいた可能性が高い。2026年7月22日、現行session・JSONL・workspaceを維持したまま実行環境だけを2026.6.6へ戻し、`context1m=true`を維持したところ、従来の発火値を超える約172kまで新たなコンパクションは起きなかった。これは問題2の旧異常閾値が現在構成で運用上解消したことを示す強い挙動証拠である。一方、2026.6.6にはPR #99864が含まれないため、問題1の累積cache usage過大計上については長いtool loopでの再発可能性を監視する。1M全域または正確な850k閾値の直接実証は未了である。**
