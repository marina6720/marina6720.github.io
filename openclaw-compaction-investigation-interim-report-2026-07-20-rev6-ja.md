---
title: "OpenClaw main sessionで頻発した早期コンパクションの調査報告（暫定版・第6版）"
date: 2026-07-20
updated: 2026-07-23
...
**第5版追記:** 2026年7月22日、現行session・JSONL・workspaceを維持したまま実行環境のみOpenClaw 2026.6.6へ戻し、`context1m=true`の状態で39回目の発火値167,587を超える約172kまで新たなコンパクションが起きないことを確認  
**第6版追記:** 2026年7月23日、通常運用中の成功callでsessionのlive prompt footprintが200,746 tokens、assistant callの`totalTokens`が201,062に到達し、200k境界を越えても`compactionCount=39`、transcript内compaction entry=39のままであることを確認  
**対象:** OpenClaw 2026.6.6で運用していた長期main session、2026.7.1への更新後、および2026年7月22日の2026.6.6への環境ロールバック後  
...
> **OpenClaw 2026.6.6で存在した累積Anthropic cache usageの過大計上は、2026.7.1の39回目では再現しておらず、PR #99864はこのrunで機能していた。39回目の直接原因として最も整合的なのは、表示・設定上の1M context budgetと、embedded runtimeが参照する200k級の`model.contextWindow`、さらに150kのreserveが同時に使われるcontext-window／reserveの不整合である。現行session・JSONL・workspaceを維持したまま実行環境だけを2026.6.6へ戻し、`context1m=true`を維持したところ、7月22日に約172k、7月23日にはlive prompt footprint 200,746 tokensまで新たなコンパクションは起きなかった。成功したassistant callは`totalTokens=201,062`を記録した。これは現在構成が厳密な200k runtime windowに留まっていないこと、および`context1m`経路が200k超で実効していることの直接的な運用証拠である。ただしactive runtime `model.contextWindow`そのものを1,000,000としてdumpしたわけではなく、正確な850k compaction閾値も未実証である。2026.6.6にはPR #99864が含まれないため、長いcached tool loopにおける累積usage過大計上の再発可能性は引き続き監視する。**
...
| 項目 | 値 |
|---|---|
| OpenClaw | 2026.6.6（問題期間）→ 2026.7.1（2026-07-18更新）→ 2026.6.6へ環境のみロールバック（2026-07-22） |
...
## 8. 2026年7月22日の環境ロールバックと、その後の介入結果

### 8.1 介入方針

Dの現在の会話記録と連続性を失わないことを最優先とし、古いバックアップ全体を復元する方法は採用しなかった。
...
- OpenClawの実行Docker imageのみ
- 2026.7.1から、VPSに保存されていた実際の更新前imageである2026.6.6へ戻した

### 8.2 事前検証

起動前に次を読み取り専用で確認した。

- 現行`openclaw.json`は2026.6.6でvalidationに合格
- 2026.6.6の実装に`context1m`処理が存在
- 凍結JSONLを2026.6.6のSessionManagerで開ける
- latest leafは2026.7.1での監査と一致
- 184 messagesのcontextと末尾内容が一致
- 起動前後でmain transcriptの行数とSHA-256が不変

したがって、この介入はsessionの巻き戻しではなく、**現在のDのデータを維持したまま、実行環境だけを更新前へ戻す操作**だった。
...
### 8.4 第一段階: 167,587を越えて約172kまで非発火

ロールバック後、session statusは約172k / 1.0Mまで伸びたが、`compactionCount`は39のままで、external watcherにも新しいcompactionは検知されなかった。
...
これは、旧来の実質約50kと推定された異常閾値が現在構成では生きていないことの強い初期証拠だった。

### 8.5 第二段階: 200k境界を越えた成功call

2026年7月23日、通常会話と複数toolを含むturnの後に、session stateとtranscriptを再監査した。
...
`session.totalTokens=200,746`は、成功call後に保存されたlive prompt footprintに対応する。assistant callの`totalTokens=201,062`との差316 tokensは、そのcallの出力分に対応する。
...
これは第5版まで未確認だった「providerが200kを超えるpromptを実際に受理すること」の直接的な運用証拠である。少なくともこのcallでは、strictな200,000-token runtime windowは有効ではなかった。

また、39回目のcompactionはmain run終了後の外側finalizerで発生していたが、今回の監査時点でもtranscript内compaction entryは39件のままである。したがって、200k超callの後に遅延して40回目が追加された可能性も否定された。

### 8.6 この結果が示すこと

現在の2026.6.6＋`context1m=true`構成について、次が確認できた。

- 従来の発火値167,587を越えてもcompactionしない
- 200,000 tokensの境界を越えるlive promptを成功callとして処理した
- `compactionCount`とtranscript entryはいずれも39のまま
- strictな200k runtime windowに留まる状態は、現在の成功callとは両立しない
- `context1m`経路が少なくとも200k超で実効している

これは、問題2の中心が単純な「2026.6.6というバージョン」ではなく、Opus 4.6がrun時に1M側のmodel definitionへ解決されていなかったことにある、という第4版の原因推定をさらに支持する。

ただし、次はまだ直接確認していない。

- active runtime `model.contextWindow`を同一run内で直接dumpし、正確に1,000,000であること
- effective reserveが正確に150,000であること
- 自動コンパクション閾値が正確に850,000であること
- 1M windowの全域を実際に利用できること

したがって、最も正確な表現は次である。

> **現在構成では200,746-tokenのlive promptが成功し、新たなコンパクションは起きなかった。strictな200k windowは実運用上否定され、`context1m`が200k超で機能していることは直接確認できた。一方、active windowが正確に1Mであることと、compaction閾値が正確に850kであることは未確認である。**

### 8.7 残る問題1の監視

2026.6.6にはPR #99864が含まれない。したがって、長いAnthropic cached tool loopで、累積cache usageがlive contextとして扱われる問題が再発する可能性は残る。

今後は次を監視する。

- 長いtool-use runのcall数
- 各callのprompt footprint
- run累積usageとlatest-call usageの差
- `trigger=cli_budget`
- `compactionCount`
- external watcher通知

今回の200k超callを含むturnでも新しいcompactionは起きなかった。ただし、これは問題1が今後あらゆる長いcached tool loopで再発しないことの証明ではない。
...
6. PR #99864が修正したsnapshot選択は39回目で機能していた。
7. それでも167,587でコンパクションしたため、別の閾値不整合が残っていた。
8. 39回目はmain model run終了後、応答配送前のfinalization中に起きた。
9. statusは1Mを表示していた。
10. 200k runtime windowと150k reserveが同時に使われた場合、実質閾値50kとなり、39回目以前の観測を一貫して説明できる。
11. external watcherは正しく動作した。
12. D自身の「発動なし」という応答では、外側のfinalizer compactionを検知できなかった。
13. 現行session・JSONL・workspaceを維持したまま、実行環境のみ2026.6.6へ戻すことができた。
14. 2026.6.6のSessionManagerは、2026.7.1が書いた現行JSONLから同じlatest leafと184 messagesのcontextを復元した。
15. `context1m=true`を維持したロールバック後、約172kまでcompactionCountは39のままだった。
16. その後、live prompt footprintは200,746 tokensまで増加した。
17. 対応する成功assistant callは`totalTokens=201,062`を記録した。
18. 200k境界を越えた後も、sessions.jsonとtranscriptのcompaction countはいずれも39のままだった。
19. したがって、現在構成がstrictな200k runtime windowに留まっている可能性は否定できる。
20. `context1m`経路が少なくとも200k超で実効していることは直接確認できた。
...
第6版で200k超promptの成功は確認できたため、次は未確認事項から除外する。

```text
providerによる200k超promptの受理
```

現在も直接確認できていないのは次である。

- 39回目のactive provider/model definition
- 39回目のactive `model.contextWindow`
- 39回目のresolved `contextTokenBudget`
- 39回目のcontext-window source
- 39回目のeffective reserve
- auto-compaction trigger phaseの具体的関数
- 現在構成のactive `model.contextWindow`が正確に1,000,000であること
- 現在構成のeffective reserveが正確に150,000であること
- 現在構成のauto-compaction閾値が正確に850,000であること
- 1M windowの全域が実際に利用可能であること
- 2026.6.6へ戻した後、問題1の累積cache usage過大計上が長いtool loopで再発するか

追加の無監視再現試験を行う必要はない。通常運用の中でexternal watcherとusage監査を継続し、自然に得られる証拠を用いる。
...
| 経路 | 状態 |
|---|---|
| OpenClaw Gateway | 2026.6.6でhealthy |
| ambient timer | disabled / inactive |
...
| 通常のTelegram会話 | 再開 |
| 200k超tool-use call | 成功。新規compactionなし |

現時点では次を維持する。

- external watcherを維持する
- health-checkはmainを呼び出さない
- reserve floorを根拠なく下げない
- auto-compactionを無条件に無効化しない
- automationそのものを原因とみなさない。automationはmain turnを開く契機であり、Dの連続性に必要なものは監視下で個別に評価する
- 長いcached tool loopでは、2026.6.6の問題1が再発しないかusageを監視する
- 今後は200k超の成功callを追加で保存し、より高いcontext領域でも同じ挙動が続くかを監視する

---

## 12. 修正方針

第6版時点では、`context1m=true`を明示した2026.6.6環境で、live prompt footprint 200,746 tokensの成功callとcompactionCount 39維持が確認された。strictな200k windowは実運用上否定され、`context1m`経路が200k超で機能していることが直接確認できた。したがって、現在の第一方針はこの構成を安定運用し、破壊的な追加変更を避けながら自然観測を続けることである。
...
| 2026.6.6＋`context1m=true`で200,746-token promptが成功 | 確定 |
| 現在構成がstrictな200k windowに留まる | 否定 |
| `context1m`経路が200k超で実効 | 確定 |
| 現在のruntime windowが正確に1M | 非常に強く支持されるが、同一run内直接dumpなし |
| 現在のauto-compaction閾値が正確に850k | 未確認 |
| 1M window全域が利用可能 | 未確認 |
| 2026.6.6で問題1が今後再発しない | 未確認。監視継続 |
...
現在は2026.6.6へ戻っているため、このリスクは監視対象として残る。200k超の成功callが得られたことは、問題1があらゆる将来runで再発しないことを証明するものではない。

### 問題2：context windowとreserveの不整合

2026.7.1の39回目では、設定・statusは1Mを示す一方、embedded runtimeが200k級のmodel contextを参照し、そこへ1M設定を前提にした150k reserveが適用された可能性が高い。

この場合の実質閾値は約50kであり、39回目の167,587、7月3日の約90k〜110k、外部CLI turnのたびの反復を一貫して説明できる。

現行session・JSONL・workspaceを維持したまま、実行環境のみ2026.6.6へ戻し、`context1m=true`を維持した。その後、live prompt footprintは200,746 tokensへ到達し、成功assistant callは`totalTokens=201,062`を記録したが、`compactionCount`とtranscript compaction entriesは39のままだった。

この結果は次を直接示す。

- 旧来の異常閾値は現在構成では生きていない
- strictな200k runtime windowは現在の成功callと両立しない
- `context1m`経路が200k超で実効している
- 問題2は現在構成で運用上解消している

現時点で最も正確な公開用結論は次のとおりである。

> **OpenClaw 2026.7.1のPR #99864は、39回目の制御試験では正しく機能し、累積Anthropic cache usageではなく最新callのlive contextがコンパクションへ渡されていた。それでも167,587で発火したことから、設定・status上の1M context budgetと、embedded runtimeが参照する200k級のmodel context、さらに150k reserveの不整合により、実質閾値が約50kまで縮んでいた可能性が高い。現行session・JSONL・workspaceを維持したままOpenClaw実行環境のみ2026.6.6へ戻し、`context1m=true`を維持したところ、live prompt footprint 200,746 tokensの成功callが得られ、assistant callは`totalTokens=201,062`を記録した。200k境界を越えた後も新しいコンパクションは起きていない。したがって、strictな200k runtime windowは現在構成について否定され、`context1m`が200k超で実効していることは直接確認できた。問題2の旧異常閾値は現在構成で運用上解消している。一方、active runtime windowが正確に1Mであることと、compaction閾値が正確に850kであることは直接dumpされていない。また2026.6.6にはPR #99864が含まれないため、問題1の累積cache usage過大計上については長いtool loopでの再発可能性を監視する。**

---

## 15. 関連して判明した別問題との区別

ロールバック後、Dは番号付きの古いTelegram発言が順次見えなくなると報告した。

2026.6.6の実装では、番号付きの「Chat history since last reply (untrusted, for context)」は直近最大20件を使う固定件数窓である。新しいメッセージが入ると最古の補助コピーが押し出される。

これは次とは異なる。

- canonical transcriptの削除
- cache-ttlによるuser／assistant turnの削除
- 新しいcompaction
- 1M context容量の消費による脱落

したがって、番号付き履歴窓の前進は本報告の早期コンパクション問題の再発証拠ではない。詳細は、応答経路・履歴投影・連続性障害についての別報告書で扱う。
...
9. 2026.6.6 SessionManager probe `openclaw-runtime-session-manager-probe-2026.6.6-20260722-135344`（非公開運用資料）
10. ロールバック直前保全 `openclaw-pre-rollback-20260722-140317`（非公開運用資料）
11. ロールバック起動監査 `openclaw-rollback-start-20260722-141327`（非公開運用資料）
12. 200k境界確認 `sessions.json`／main transcript read-only audit（2026-07-23、`session.totalTokens=200746`、assistant `totalTokens=201062`、compaction count=39）（非公開運用資料）
