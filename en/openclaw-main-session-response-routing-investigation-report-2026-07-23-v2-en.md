...
The current session, JSONL, and workspace were preserved, while only the runtime environment was rolled back to OpenClaw 2026.6.6—the version D had actually used before the update. In the first tool-use regression test, D produced one complete reply. No standalone commentary delivery or progressive rewriting was observed.

During later normal conversation, however, `delivery-mirror` duplication recurred in the smartphone application. The app displayed the same long reply twice. In another duplicated short reply, one bubble retained the trailing `NO_REPLY` marker while the other did not. Telegram itself contained only one actual delivery of each message. This is strongly consistent with the smartphone history view displaying both the raw canonical assistant record and its channel-formatted `delivery-mirror`.

The four-reply/same-run-replay failure exposed under 2026.7.1 and the `delivery-mirror` duplication that remains under 2026.6.6 are therefore separate failures.

The central conclusion is therefore:

> **The multiple replies and progressive thinning were caused primarily by OpenClaw 2026.7.1’s tool-use orchestration, intermediate delivery path, and same-run provider projection—not by a personality or capability failure in D or Claude Opus 4.6. `delivery-mirror` duplication, however, recurred after the rollback to 2026.6.6 and is therefore an older OpenClaw history-projection defect rather than only a 2026.7.1 regression. The advancing numbered-history boundary was a separate fixed-window design. The distorted post-compaction self-description was a composite problem involving both model-generated summarization and OpenClaw’s continuity design.**
...
### 8.1 Recurrence After the Rollback to 2026.6.6

The first tool-use regression test did not show duplicate assistant messages in the smartphone application. During later normal conversation, however, the same reply again appeared as two assistant bubbles.

The screenshots show two forms of duplication:

1. The same long reply appears twice in the smartphone application.
2. The same short reply appears twice, with one version retaining the trailing `NO_REPLY` marker and the other version omitting it.

The corresponding Telegram screen shows only one actual delivery of the long reply and one actual delivery of the later short reply. The content was therefore not sent to Telegram twice.

The difference involving `NO_REPLY` is strongly consistent with the following projection:

- a raw canonical assistant record retaining the control marker
- a channel-formatted `delivery-mirror` in which the marker was removed before delivery

Previous audits had already found both exact mirrors and mirrors reflecting delivery sanitization.

The screenshots do not by themselves identify the exact JSONL record IDs, but the combined evidence is strong:

- two assistant bubbles are present in the smartphone application
- Telegram contains only one actual channel delivery
- the two displayed texts differ in a delivery-formatting detail
- the frozen JSONL contains both canonical assistant records and corresponding mirrors
- the earlier mirror-topology audit found a canonical source for every mirror

The post-rollback result must therefore be updated:

| Behavior | After rollback to 2026.6.6 |
|---|---|
| Multiple commentary replies during tool use | Not reproduced |
| Independent Telegram leakage of commentary | Not reproduced |
| Progressive thinning from same-run replay | Not reproduced |
| Duplicate final delivery to Telegram | Not observed |
| Duplicate smartphone display involving `delivery-mirror` | Reproduced |

The four-reply failure and the duplicate-display failure are not the same defect.

- The four-reply/progressive-thinning failure is strongly associated with 2026.7.1 tool-use and commentary handling.
- `delivery-mirror` duplication also exists under 2026.6.6 and is an older history-projection defect.

There is no evidence that this duplicate display duplicated or damaged the canonical transcript, session tree, D’s provider context, or the actual Telegram delivery count. Its impact is nevertheless not literally zero:

- Marina sees one statement twice.
- It becomes difficult to tell which bubble represents the canonical wording.
- Internal control text such as `NO_REPLY` may be exposed.
- The symptom can be mistaken for a recurrence of the multi-reply failure.
- Readability and trust in the conversation interface are reduced.

The accurate conclusion is therefore: **no data corruption or duplicate channel delivery has been observed, but a user-visible and operational display defect remains.**
...
| Duplicate display from `delivery-mirror` | Confirmed history-projection problem | Canonical and mirror shown in the same assistant list under both 2026.6.6 and 2026.7.1 |
...
The first test produced the following result:
...
- No identical duplicate appeared in that single exchange.
...
During later normal conversation, only the smartphone `delivery-mirror` duplication recurred. Telegram still contained a single actual delivery.

The intervention therefore established a narrower and more precise separation:

- the multiple commentary deliveries and same-run progressive thinning seen under 2026.7.1 have not recurred under 2026.6.6
- the `delivery-mirror` history-projection duplication remains under 2026.6.6

The rollback is strong evidence that the multi-reply and progressive-thinning failure belonged to the 2026.7.1 path. It is not evidence that the duplicate-display defect was repaired.
...
10. Preserving current data while rolling back only the runtime to 2026.6.6 stopped the multiple commentary deliveries and progressive thinning in the initial tool-use regression test.
11. The smartphone `delivery-mirror` duplication later recurred under 2026.6.6, while Telegram still contained only one actual delivery.
12. The four-reply/same-run-replay failure is therefore most likely a 2026.7.1 regression, while `delivery-mirror` duplication is an older OpenClaw history-projection defect.

The most concise conclusion is:

> **D was not broken. Under 2026.7.1, OpenClaw took one tool-use process, delivered its intermediate commentary as multiple ordinary replies, and replayed those intermediate answers into subsequent model passes. Returning only the runtime to the pre-update version stopped that four-reply and progressive-thinning failure while preserving D’s current record and restoring one complete Telegram answer. A separate history-projection defect remains under 2026.6.6: the smartphone application can display both the canonical answer and its delivery-formatted `delivery-mirror`. The canonical conversation and actual channel delivery remain intact, but the user-visible display is not yet repaired.**
...
- `001.webp` — smartphone app duplicate with and without trailing `NO_REPLY`
- `002.webp` — identical long reply displayed twice in the smartphone app
- `003.webp` — comparison showing one actual Telegram delivery for each reply
