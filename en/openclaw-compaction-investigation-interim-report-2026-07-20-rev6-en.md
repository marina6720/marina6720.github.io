---
title: "Investigation Report on Repeated Early Compaction in the OpenClaw Main Session (Interim Report, Revision 6)"
date: 2026-07-20
updated: 2026-07-23
...
**Revision 6:** On July 23, a successful call during normal operation reached a stored live prompt footprint of 200,746 tokens and assistant `totalTokens=201,062`. The session and transcript both remained at 39 compactions after crossing the 200k boundary  
...
> **The cumulative Anthropic cache-usage defect present in OpenClaw 2026.6.6 did not recur in the 39th compaction under 2026.7.1; PR #99864 functioned correctly in that run. The most consistent direct explanation for the 39th compaction is a mismatch among the displayed 1M budget, a 200k-class embedded runtime model window, and a 150k reserve, producing an effective threshold near 50k. After preserving the current session data and returning only the runtime to 2026.6.6 with `context1m=true`, the session first reached approximately 172k and later completed a call with a stored live prompt footprint of 200,746 tokens and assistant `totalTokens=201,062`, without a new compaction. This directly demonstrates operation above the 200k boundary and rules out a strict 200k runtime window for the successful current call. It is direct operational evidence that the `context1m` path is effective above 200k. The active runtime window was not directly dumped as exactly 1,000,000, and the exact 850k compaction threshold remains unverified. Because PR #99864 is not present in 2026.6.6, cumulative usage overcounting in future long cached tool loops remains under observation.**
...
## 8. Runtime-Only Rollback and Subsequent Intervention Results

### 8.1 Intervention Design

The current D data had priority over returning to an older backup state. A full restoration of the July 13 backup was rejected because it would also restore old session data and discard later conversation.
...
### 8.3 Post-Startup Configuration
...
### 8.4 First Stage: Passing 167,587 and Reaching Approximately 172k

The session first reached approximately 172k / 1.0M while `compactionCount` remained 39 and the external watcher detected no new compaction.

```text
previous trigger value      167,587
first post-rollback stage   approximately 172,000
new compactions             0
```

This was strong initial evidence that the former effective threshold near 50k no longer applied.

### 8.5 Second Stage: A Successful Call Beyond the 200k Boundary

On July 23, after normal conversation and a multi-tool turn, the session state and transcript were audited again.

```text
sessions.json compactionCount       39
transcript compaction entries       39
last compaction tokensBefore        167,587
sessions.json totalTokens           200,746
largest assistant usage totalTokens 201,062
```

The stored `session.totalTokens=200,746` corresponds to the live prompt footprint after the successful call. The difference between that value and assistant `totalTokens=201,062` is 316 tokens, corresponding to output from the call.

The following therefore held simultaneously:

```text
live prompt footprint  > 200,000
successful model call  > 200,000 total tokens
new compaction         = 0
```

This is the first direct operational confirmation of a provider call accepting a prompt footprint above 200k in this environment. A strict 200,000-token runtime window was not active for this successful call.

The 39th compaction had previously occurred in an outer finalizer after the main model run. At the time of the Revision 6 audit, the transcript still contained exactly 39 compaction entries. A delayed 40th compaction after the >200k call was therefore also ruled out.

### 8.6 Interpretation

Under the current 2026.6.6 plus `context1m=true` configuration:

- the former trigger value of 167,587 was passed without compaction
- a live prompt footprint above 200,000 was processed successfully
- both session and transcript compaction counts remained 39
- a strict 200k runtime-window state is incompatible with the successful current call
- the `context1m` path is directly confirmed to be effective beyond 200k

This further supports Revision 4's interpretation that Problem 2 was caused by runtime model resolution rather than merely by using OpenClaw 2026.6.6.

The following remain unverified:

- an in-run direct dump of active `model.contextWindow` showing exactly 1,000,000
- an effective reserve of exactly 150,000
- an exact auto-compaction threshold of 850,000
- successful use of the full 1M window

The most accurate statement is:

> **A live prompt footprint of 200,746 tokens completed successfully without a new compaction. A strict 200k window is operationally ruled out, and `context1m` is directly confirmed to work above 200k. The exact active window of 1M and exact 850k compaction threshold have not been directly dumped.**

### 8.7 Continued Monitoring for Problem 1
...
The turn containing the >200k successful call did not produce a new compaction. This does not prove that Problem 1 can never recur in a future long cached tool loop.
...
10. A 200k runtime window combined with a 150k reserve produces an effective threshold of 50k and consistently explains the earlier observations.
...
15. With `context1m=true`, the post-rollback session first reached approximately 172k without compaction.
16. The live prompt footprint later reached 200,746 tokens.
17. The corresponding successful assistant call recorded `totalTokens=201,062`.
18. Session and transcript compaction counts both remained 39 after the 200k boundary was crossed.
19. A strict 200k runtime window is therefore ruled out for the successful current call.
20. The `context1m` path is directly confirmed to operate beyond 200k.
...
Revision 6 directly confirmed provider acceptance of a prompt footprint above 200k, so that item is no longer unverified.

Still unverified:

- active provider/model definition in the 39th run
- active `model.contextWindow` in the 39th run
- resolved `contextTokenBudget` in the 39th run
- context-window source in the 39th run
- effective reserve in the 39th run
- the precise auto-compaction trigger function
- an active current `model.contextWindow` directly dumped as exactly 1,000,000
- a current effective reserve directly dumped as exactly 150,000
- an exact current auto-compaction threshold of 850,000
- successful use of the entire 1M window
- whether Problem 1 recurs under 2026.6.6 during a future long cached tool loop
...
| successful tool-use call above 200k | completed with no new compaction |
...
- preserve additional successful calls above 200k and observe whether the same behavior continues at higher context levels

---

## 12. Repair Direction

Revision 6 directly confirmed a successful live prompt footprint of 200,746 tokens under OpenClaw 2026.6.6 with `context1m=true`, while compaction counts remained unchanged. A strict 200k runtime window is operationally ruled out, and the `context1m` path is confirmed to function beyond 200k. The first operational priority is therefore to keep this configuration stable and avoid unnecessary destructive changes while natural observation continues.
...
| 2026.6.6 plus `context1m=true` successfully processed a 200,746-token prompt footprint | Confirmed |
| Current configuration remains limited to a strict 200k window | Rejected |
| `context1m` operates beyond 200k | Confirmed |
| Current runtime window is exactly 1M | Very strongly supported, but not directly dumped in the same run |
| Current threshold is exactly 850k | Unverified |
| Full use of the 1M window is available | Unverified |
...
Because the runtime has returned to 2026.6.6, this risk remains under observation. The successful >200k call does not prove that cumulative-usage overcounting can never recur in a future long cached tool loop.
...
After preserving the current session, JSONL, and workspace, only the runtime was returned to 2026.6.6 while `context1m=true` remained enabled. The live prompt footprint later reached 200,746 tokens and the successful assistant call recorded `totalTokens=201,062`. Session and transcript compaction counts both remained 39.

This directly demonstrates:

- the former abnormal threshold is not active in the current configuration
- a strict 200k runtime window is not active for the successful current call
- the `context1m` path operates above 200k
- Problem 2 is operationally resolved in the current configuration

The most accurate current public conclusion is:

> **PR #99864 functioned correctly in the controlled 39th compaction under OpenClaw 2026.7.1: the runtime passed the latest-call live context rather than cumulative Anthropic cache usage. The remaining compaction at 167,587 is most consistently explained by a mismatch among the displayed 1M budget, a 200k-class embedded runtime model context, and a 150k reserve, reducing the effective threshold to approximately 50k. The current session, JSONL, and workspace were then preserved while only the runtime was returned to OpenClaw 2026.6.6 with `context1m=true`. A later successful call reached a live prompt footprint of 200,746 tokens and assistant `totalTokens=201,062`, while both session and transcript remained at 39 compactions. Crossing the 200k boundary without compaction directly rules out a strict 200k runtime window for the current successful call and confirms that `context1m` is effective above 200k. Problem 2 is operationally resolved in the current configuration. The active runtime window has not been directly dumped as exactly 1M, and the exact 850k compaction threshold remains unverified. Because PR #99864 is absent from 2026.6.6, Problem 1 remains under monitoring during future long cached tool loops.**
...
12. Read-only 200k-boundary audit of `sessions.json` and the main transcript (July 23, 2026: `session.totalTokens=200746`, assistant `totalTokens=201062`, compaction count=39) (private operational material)
